...
import useDM from '../../chat/hooks/useDM';
...
export default function SettingsPage() {
  ...
  const sectionParam = (params?.section as string) || 'profile';
  ...
  const [savingField, setSavingField] = useState<'name' | 'username' | null>(null);
  ...
  async function saveProfileData(body: Record<string, any>, successMsg: string, resetEditFn?: () => void) {
    setProfileErr('');
    setProfileSuccess('');
    setProfileBusy(true);
    try {
  ...
  function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      saveProfileData({ profileImage: result }, 'Profile picture updated!');
    };
  ...
  async function handleSaveUsername() {
    ...
    if (usernameStatus === 'taken') {
      setProfileErr('Please choose an available username');
      return;
    }
  ...
  function handleToggleOnlineStatus() {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('settings_showOnline', String(next));
  ...
  function handleToggleDP() {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('settings_showDP', String(next));
    setPrivacySuccess('Privacy settings saved');
    setTimeout(() => setPrivacySuccess(''), 3000);
  }

  function handleAddBlocked(e: React.FormEvent) {
    e.preventDefault();
    ...
    if (typeof window !== 'undefined') localStorage.setItem('settings_blocked', JSON.stringify(updated));
    setBlockInput('');
    setPrivacySuccess(`Blocked @${target}`);
    setTimeout(() => setPrivacySuccess(''), 3000);
  }

  function handleUnblock(target: string) {
  ...
  async function updateNotifSettings(payload: Record<string, any>) {
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('accessToken') : '';
  ...
  function handleToggleGroupNotif() {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('settings_groupNotif', String(next));
  ...
  function handleToggleDmNotif() {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('settings_dmNotif', String(next));
  ...
  function handleToggleBgSync() {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('settings_bgSync', String(next));
  ...
  const [mobileActiveView, setMobileActiveView] = useState<'sidebar' | 'chat'>(() =>
      ...
      <nav className="nav-rail">
        <div className="rail-top">
          <button
            ...
            onClick={() => router.push('/channels?openCreate=true')}
          ...
          <button
            className="rail-btn"
            onClick={() => router.push('/channels?openSearch=true')}
        ...
        <div className="rail-bottom">
          <button className="rail-btn settings-btn active" onClick={() => { setMobileActiveView('sidebar'); router.push('/settings/profile'); }} title="Settings">
