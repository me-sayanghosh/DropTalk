...
export default function JoinNow() {
  ...
  const [loadingStatusText, setLoadingStatusText] = useState('AUTHENTICATING WITH GOOGLE…');
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setLoadingStatusText('VERIFYING GOOGLE ACCOUNT…');
      try {
        ...
        setLoadingStatusText('INITIALIZING SECURE SESSION…');
        const backendRes = await fetch(`${API_BASE}/auth/google-direct`, {
          method: 'POST',
        ...
        setLoadingStatusText('LAUNCHING HOME CHAT…');
    ...
    onError: () => {
      setBusy(false);
      setAuthSuccessLoading(false);
      setErr('Google Sign-In was cancelled or blocked by browser settings.');
    },
  });
  ...
  async function verifyOtpCode(codeToVerify?: string) {
    ...
    setBusy(true);
    setAuthSuccessLoading(true);
    setLoadingStatusText('VERIFYING SECURITY CODE…');
    try {
  ...
  return (
    <div className="bauhaus-auth-page">
      ...
      >
        ...
        {authSuccessLoading ? (
          <div className="bauhaus-auth-loading-content">
            <div className="bauhaus-loading-shapes">
              <span className="shape-circle" />
              <span className="shape-square" />
              <span className="shape-triangle" />
            </div>
            <h2 className="bauhaus-loading-title">{loadingStatusText}</h2>
            <p className="bauhaus-loading-subtitle">
              ESTABLISHING ENCRYPTED WORKSPACE // PLEASE WAIT
            </p>
            <div className="bauhaus-loading-progress-bar">
        ...
        <div className="bauhaus-auth-google-wrap">
          ...
          >
            {busy && !authSuccessLoading ? (
              <span className="flex items-center gap-2">
                <div className="lazy-spinner" style={{ width: '16px', height: '16px', borderWidth: '2px' }} />
